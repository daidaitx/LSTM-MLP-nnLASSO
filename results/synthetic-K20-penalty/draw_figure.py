...
def plot_shifted_losses_from_cases(cases, results_dir, fig_name):
    ...
    # 定义颜色映射（增强对比度版本）
    color_map = {
        'linear': ["#0016a8", "#1980d0", "#7ac3fe"],  # 深海军蓝 -> 中蓝 -> 浅天蓝
        'quadratic': ["#004605", '#43a047', '#a5d6a7'],  # 深森林绿 -> 中绿 -> 浅薄荷绿
        'exponential': ["#ae0000", "#ef5c59", "#ffb8bf"]  # 深酒红 -> 中红 -> 浅珊瑚红
    }
    ...
    for losses, label, violation in zip(all_losses, case_labels, max_violations):
        ...
        # 根据案例类型和参数选择颜色
        if 'linear' in label:
            color_idx = min(2, int(float(label.split(' ')[0].split('-')[-1]) * 100 - 1))
            color = color_map['linear'][color_idx]
        elif 'quadratic' in label:
            color_idx = min(2, int(float(label.split(' ')[0].split('-')[-1]) * 10 - 1))
            color = color_map['quadratic'][color_idx]
        elif 'exponential' in label:
            color_idx = min(2, int(float(label.split(' ')[0].split('-')[-1]) * 100 - 1))
            color = color_map['exponential'][color_idx]
...
# 指定需要绘制的案例名称
cases_to_plot = ["nnLASSO-linear-penalty-0.01", "nnLASSO-linear-penalty-0.02", "nnLASSO-linear-penalty-0.03",
                 "nnLASSO-quadratic-penalty-0.1", "nnLASSO-quadratic-penalty-0.2", "nnLASSO-quadratic-penalty-0.3",
                 "nnLASSO-exponential-penalty-0.01", "nnLASSO-exponential-penalty-0.02", "nnLASSO-exponential-penalty-0.03"]

# 整合约束违反度数据
aggregate_violations(cases_to_plot, 'results/synthetic-K20-penalty', 'nnLASSO-violations-summary.csv')

# 调用函数绘制图形
plot_shifted_losses_from_cases(cases_to_plot, 'results/synthetic-K20-penalty', 'synthetic-K20-penalty_loss.png')