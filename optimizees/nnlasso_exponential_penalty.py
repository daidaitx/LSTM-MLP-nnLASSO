...
class nnLASSO_exponential_penalty(BaseOptimizee):
    ...
    def __init__(
        ...
        **options
        ...
        优化问题形式:
            minimize (1/2) * ||Y - W @ X||_2^2 + pen*sum( max(0, exp(-X)-1) ) + rho * ||X||_1
               X
        """
    ...
    def objective(self, inputs: dict = None, compute_grad: bool = False):
        ...
        with torch.set_grad_enabled(compute_grad):
            ...
            l_penalty = self.pen * torch.where(X > 0, torch.zeros_like(X), torch.exp(-X) - 1).sum(dim=(1,2)).mean()
            return l1 + l2 + l_penalty
    ...
    def objective_batch(self, inputs: dict = {}, compute_grad: bool = False):
        ...
        with torch.set_grad_enabled(compute_grad):
            ...
            l_penalty = self.pen * torch.where(X > 0, torch.zeros_like(X), torch.exp(-X) - 1).sum(dim=(1,2))
            return l1 + l2 + l_penalty
    ...
    def smooth_grad(self, inputs: dict = None, compute_grad: bool = False, **kwargs):
        ...
        with torch.set_grad_enabled(compute_grad):
            ...
            l1 = torch.bmm(W.permute(0,2,1), residual)
            l_penalty = self.pen * torch.where(X > 0, torch.zeros_like(X), -torch.exp(-X))
            return l1 + l_penalty
