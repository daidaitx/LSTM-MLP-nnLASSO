...
class nnLASSO_linear_penalty(BaseOptimizee):
    ...
    def __init__(
        ...
        **options
        ...
        优化问题形式:
            minimize (1/2) * ||Y - W @ X||_2^2 + pen*sum(relu(X)-X) + rho * ||X||_1
               X
        """
    ...
    def objective(self, inputs: dict = None, compute_grad: bool = False):
        ...
        with torch.set_grad_enabled(compute_grad):
            ...
            l_penalty = self.pen * (torch.relu(X) - X).sum(dim=(1,2)).mean()
    ...
    def objective_batch(self, inputs: dict = {}, compute_grad: bool = False):
        ...
        with torch.set_grad_enabled(compute_grad):
            ...
            l_penalty = self.pen * (torch.relu(X) - X).sum(dim=(1,2))
            return l1 + l2 + l_penalty
    ...
    def smooth_grad(self, inputs: dict = None, compute_grad: bool = False, **kwargs):
        ...
        with torch.set_grad_enabled(compute_grad):
            ...
            l_penalty = torch.where(X < 0, torch.full_like(X, -self.pen), torch.full_like(X, 0))
            return l2 + l_penalty
